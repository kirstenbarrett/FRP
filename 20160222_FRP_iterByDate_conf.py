...
while datIter < len(datList):
 ...
 #       data = data[1472:1546,566:656] #BOUNDARY FIRE AREA
    ...
    if np.max(allFires) > 0:
        ...
        exportCSV = np.column_stack([FRPx,FRPy,FRPxProj,FRPyProj,Area,FRP,FrpArea,hrs,mints,js,yrs,detnConf])
        np.savetxt(filNam+'iterByDate_conf.csv', exportCSV, delimiter=",")
...
##ADD HEADER TO FILES
##filList = os.listdir('.')
##hdr = 'X,Y,AEA_AK_X,AEA_AK_Y,Area,FRP,FrpArea,hr,min,julian,year,filNam\n'
